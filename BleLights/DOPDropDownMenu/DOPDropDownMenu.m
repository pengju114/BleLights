...
#define kTableViewCellHeight 32
...
#define kCellBgColor [UIColor clearColor]
...
//    }
    ...
    if (_titles) {
        for (CALayer *l in _titles) {
            [l removeFromSuperlayer];
        }
    }
    
    if (_bgLayers) {
        for (CALayer *l in _bgLayers) {
            [l removeFromSuperlayer];
        }
    }
    
    if (_indicators) {
        for (CALayer *l in _indicators) {
            [l removeFromSuperlayer];
        }
    }
    ...
    CGFloat textLayerInterval = 26;//self.frame.size.width / ( _numOfMenu * 2);
...
- (CGSize)calculateTitleSizeWithString:(NSString *)string
{
    ...
    if ([string respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
        size = [string boundingRectWithSize:CGSizeMake(280, 0) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:dic context:nil].size;
    }else{
        size = [string sizeWithFont:[UIFont systemFontOfSize:_fontSize]];
    }
