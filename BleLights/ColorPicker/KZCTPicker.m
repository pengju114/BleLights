...
- (void) setup
{
	...
	// HS wheel
	KZCTPickerHSWheel *wheel = [[KZCTPickerHSWheel alloc] initAtOrigin:CGPointMake((self.frame.size.width - 240) * 0.5, 4)];
...
- (void) setSelectedColor:(UIColor *)c
{
    ...
    RGBType rgb = [self rgbWithUIColor:c];
...
- (void) colorWheelColorChanged:(KZColorPickerHSWheel *)wheel
{
	...
	self.selectedColor = [UIColor colorWithHue:hsv.h
									...
									brightness:self.brightnessSlider.value
										 alpha:1/*self.alphaSlider.value*/];
...
- (void) fixLocations
{
    ...
    CGRect r = self.colorWheel.frame;
    r.origin.x = (self.frame.size.width - r.size.width) * 0.5;
    self.colorWheel.frame = r;
    
    CGFloat topHeight = r.origin.y + r.size.height;
    CGFloat space = self.frame.size.height - topHeight;
    
    r = self.brightnessSlider.frame;
    r.origin.x = (self.frame.size.width - r.size.width) * 0.5;
    r.origin.y = topHeight + (space - r.size.height) * 0.5f;
    self.brightnessSlider.frame = r;
    //horizontal
...
//        }
//        
//        self.brightnessSlider.frame = CGRectMake((self.frame.size.width - 272) * 0.5,
//                                                 277,
//                                                 272,
//                                                 24);
//        
