...
{
    return [self initAtOrigin:origin withImage:@"pickerColorWheel"];
}

-(id)initAtOrigin:(CGPoint)origin withImage:(NSString *)img{
    if ((self = [super initWithFrame:CGRectMake(origin.x, origin.y, 240, 240)]))
    {
        // Initialization code
        // add the imageView for the color wheel
        UIImageView *wheel = [[UIImageView alloc] initWithImage:[UIImage imageNamed:img]];
...
-(void)layoutSubviews{
    ...
    CGRect rect = self.wheelImageView.frame;
    rect.origin.x = (self.frame.size.width - rect.size.width) * 0.5f;
    self.wheelImageView.frame = rect;
}
